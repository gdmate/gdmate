...
def import_prm_library(prm_library=None) -> None:
    """
    Function to import the default parameter libray, as a dictionary, from the parameters.xml at
...
def prmname_check(prm_dict: dict, prm_library=None) -> None:
    """
    Function to check if all parameter names in a parameter dictionary are recognised.

    Unrecognised parameters are printed on the screen.
    ...
    def lst_recu(prm_dict, prm_library, tmp_list=None) -> list:
        """
        Function to create a list of unrecognised parameters
